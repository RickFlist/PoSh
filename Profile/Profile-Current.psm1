...
"@
        ...
        foreach ( $arpEntry in $ArpResults.GetEnumerator() )
        {
            ...
            if ( $netAdpaterObj )
            {
                $nicName = $netAdpaterObj.InterfaceDescription
            }
        ...
        $iFaces | ForEach-Object {
            ...
            foreach ($dnsIp in $PSItem.DnsServerIps)
            {
                ...
                foreach ($name in $DnsName)
                {
                    ...
                    if ( -not $Quiet.IsPresent )
                    {
                        Write-Host  ("`t`t`t`tName: {0,$dnsNamePadding}: DNS Server {1}" -f $name, $dnsIp) -ForegroundColor Green
                    }
                    try
                    {
...
function Add-OpenWithProgramToContextMenu
{
    [CmdletBinding( DefaultParameterSetName = 'CustomApp')]
    [OutputType()]
    ...
    (
        ...
        [ValidateNotNullOrEmpty()]
        # Path to program to add to context menu
        [System.IO.FileInfo]
        $LiteralPath
        ,
        [Parameter( ParameterSetName = 'CustomApp' )]
        [ValidateNotNullOrEmpty()]
        # Name to display in the context menu for the app. If not specified it will use [FileInfo].VersionInfo.FileDescription if availble, and will utltimately fall back to [FileInfo].BaseName
        [String]
        $DisplayName
        ,
        [Parameter( ParameterSetName = 'CustomApp' )]
        [ValidateLength(1, 1)]
        [ValidateNotNullOrEmpty()]
        # Letter within the display name to press on the keyboard to quickly select this item from the context menu
        [String]
        $HotKey
    )
    ...
    process
    {
        #region Verify-Access-To-Application-Executable
        ...
        $LiteralPath.Refresh()
        #endregion Verify-Access-To-Application-Executable

        #region Set-DisplayName
        if ( -not ( $PSBoundParameters.ContainsKey( 'DisplayName' ) ) )
        {
            if ( $LiteralPath.VersionInfo.FileDescription )
            {
                $DisplayName = ( ( $LiteralPath.VersionInfo.FileDescription.Split( ':', [System.StringSplitOptions]::RemoveEmptyEntries ) )[0].Trim() )
            }
            else
            {
                $DisplayName = ( ConvertTo-TitleCase -InputObject $LiteralPath.BaseName )
            }
        }
        #endregion Set-DisplayName

        #region Verify-PSDrive-for-Registry-Key
        $regRawRoot = ( 'HKEY_CLASSES_ROOT' )
        $psProviderName = ( 'HKCR' )
        $hkcrExists = ( Get-PSDrive | Where-Object { $PSItem.Root -eq $regRawRoot } )
        if ( -not $hkcrExists )
        {
            try
            {
                $null = ( New-PSDrive -Name $psProviderName -PSProvider Registry -Root $regRawRoot )
            }
            catch
            {
                throw ( $PSItem )
        ...
        #endregion Verify-PSDrive-for-Registry-Key

        #region Verify-Key-Existence
        $regPathSuffix = ( '*\shell' )
        $regRoot = ( Join-Path -Path ( '{0}:' -f $psProviderName ) -ChildPath $regPathSuffix )
        $regRootObj = ( Get-Item -LiteralPath $regRoot )

        $regKeyObj = ( Get-ChildItem -LiteralPath $regRoot | Where-Object { $PSItem.Name -like ( '*{0}*' -f $DisplayName ) } )
        if ( $regKeyObj )
        {
            $padSize = 0
            $regKeyObj.Property | ForEach-Object {
                if ( $PSItem.Length -gt $padSize )
                {
                    $padSize = $PSItem.Length
                }
            }

            Write-Host ( 'A key already exists named "{0}" containing a portion of the Display Name. The key values are:' -f $regKeyObj.PSChildName )
            ( $regKeyObj.Property | ForEach-Object {
                    ( [System.Management.Automation.PSCustomObject] @{
                            KeyName  = $PSItem
                            KeyValue = ( Get-ItemPropertyValue -LiteralPath $regKeyObj.PSPath -Name $PSItem )
                        } )
                } | `
        ...
        #endregion Verify-Key-Existence

        #region Create-Registry-Entries
        $regAddRootPath = ( 'HKCR\*\shell' )
        $regAddPathOne = ( '{0}\Open with {1}' -f $regAddRootPath,$DisplayName )
        $regAddPathTwo = ( '{0}\command' -f $regAddPathOne )

        $allCmds = @()
        $allCmds += ( 'ADD "{0}" /ve /d "Open with {1}" /f' -f $regAddPathOne,$DisplayName )
        $allCmds += ( 'ADD "{0}" /v Icon /d "{1},0" /f' -f $regAddPathOne,$LiteralPath.FullName )
        $allCmds += ( 'ADD "{0}" /ve /d ""{1}" "%1"" /f' -f $regAddPathTwo,$LiteralPath.FullName )

        foreach ( $cmd in $allCmds )
        {
            Write-Debug ( 'Reg Cmd: {0}' -f $cmd )
            Start-Process -FilePath ( Get-Command -Name reg.exe ).Path -ArgumentList $cmd
        }

        #endregion Create-Registry-Entries
    }
}
...
function Show-MethodDefinition
{
    [CmdletBinding()]
    [OutputType()]
    ...
    process
    {
        $allMethods = ( New-Object -TypeName System.Collections.ArrayList )
        foreach ( $method in $InputObject.PSObject.Methods.GetEnumerator() )
        {
            $null = $allMethods.Add( [PSCustomObject] @{
                Name = ( $method.Name )
                Definitions = ( $method.OverloadDefinitions -join "`r`n" )
            } )
        }

        ( $allMethods | Format-Table -Autosize -Wrap )
    }
}
